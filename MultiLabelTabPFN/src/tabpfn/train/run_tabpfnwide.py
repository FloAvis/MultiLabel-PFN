...
class Trainer:
    ...
    def __init__(self, parsed_args=defaultdict(dict)):
        ...
        self.configure_ddp()
        self.prior_dataset_config = PriorDatasetConfig(batch_size=self.batch_size,
                                                       **parsed_args["prior_dataset_config"])
        self.prior_dataloader_config = PriorDataLoaderConfig(pin_memory_device=self.device,
                                                             **parsed_args["prior_dataloader_config"])
        self.load_model()
        self.configure_amp()
        self.start_time = datetime.datetime.now()
        self.curr_step = 0
        self.dataloader = iter(
            load_prior_dataloader(PriorDataset, self.prior_dataset_config, self.prior_dataloader_config))
    ...
    def load_model(self):
        ...
        self.model = model_
        self.model.train()
        self.optimizer = AdamW(self.model.parameters(), lr=self.train_config.learning_rate,
    ...
    def validate(self):
        ...
        pred_res = []
        val_losses = []
        for dataset in get_wide_validation_data(self.device, self.train_config.validation_datasets,
                                                self.train_config.omic_combinations):
    ...
    def train(self):
        ...
        for i in step_progress:
            ...
            if not (torch.all(d == d[0]) and torch.all(seq_len == seq_len[0]) and torch.all(
                    trainsizes == trainsizes[0])):
                continue
            X = X[:, :, :d[0]]
