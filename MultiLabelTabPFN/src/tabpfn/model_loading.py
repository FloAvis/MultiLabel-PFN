...
def get_n_out(
    ...
    if config.max_labels == 2:
        return 1
    if config.max_labels > 2 and isinstance(loss, nn.BCEWithLogitsLoss):
        return config.max_labels
    raise ValueError(
        "Unknown configuration: "
        f"max_labels={config.max_labels} and loss={type(loss)}"
    )
